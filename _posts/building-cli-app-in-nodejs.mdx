...
---
In this blog we are going to build a simple Cli app that lists and shows the content of a directory. We will use Node.js to build this app. 
...
pnpm install
```
When running node on the console, anything passed while executing the file could be accessed from the `process.argv` array. The first element of the array is the path to the node executable whle the second element is the path to the file that is being executed. The rest of the elements in the array are the arguments passed to the file. 
For example if we run the following command on the console
```bash
...
[
    '/usr/local/bin/node',
    '/Users/henoktsegaye/Desktop/node-ts-starter/index.js',
    '--version'
]
```
We can use this array to parse the arguments passed to the file and do whatever we wanted to do based on the arguments passed. For example, if we want to print the version of the app we can do something like this
```js
...
### Using npm packages
Using the `process.argv` array to parse the arguments passed to the file could be a bit tedious and error prone if we have a lot of arguments to parse.
![error prone](https://media.giphy.com/media/bAPfBDRF4Hg0U/giphy.gif)
There are a couple of great npm packages that could actully help us write this cli app faster but we could choice not to use those packages and use write our own parser instead.
 In this blog we are going to use some of those packages to help us write this app faster.
...
A CLI app is different in the way that the user could interact with the app, there is no UI or nice interface the user could click around. Instead users have to type commands to do everything , to even get help. Comamnder.js could actully help us on this. Commander handles parsing input from the command line that the user types and helps us to define custom commands and options for our app.
...
```
Now that we have the basics we can call `parse` method on the program instance to parse the input from the command line.
```ts
...
# output 0.0.1
```
Now let's define an option that lists the content of a directory. we can do that by calling the `option` method on the program instance.
```ts
program.
version('0.0.1').
description('A simple cli app that lists and shows the content of a directory').
...
```
Now that we have defined the options for the CLI we can start to implement the logic for each option. 
```ts
import fs from 'fs';
