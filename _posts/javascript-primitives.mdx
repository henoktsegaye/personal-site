...
Anything other than primitive types in JavaScript is an object. Functions, classes, or arrays are all objects.
...
Changing the value of `a` will not change the value of `b` since they are pointing to different values in the stack.At this point **GC( Garbage Collector)** will remove unreferenced values from the stack.
...
``` 
As you can see in the above example, Objects don't hold values but references so, changing `a` will change `b ` as they have the same reference.
...
This is actually one of the features of javascript. 

Primitives are not Objects but they **act like they are** 🤔 ? Well here is how it works
