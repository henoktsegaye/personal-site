---
date: "2022-09-11"
thumbnail: /assets/blog/Numbers/cover.png
title: Number types in Javascript
description: Javascript Numbers are primitive types and represent all number types in Javascript that include integer and floating numbers but in reality every number is represented as floating number in javascript.
hashtag: Javascript
...
Any numeric value in javascript is represent by the **Number** primitive data type and its represented as a floating number.

This is regardless the size of the number( how may bits the number take to store the value), the number sign (positive or negative ) whether or not the number has
...
Note that when staring with 0 the number would be interpreted as octal when each number digit is between 0 and 7, if not it would be interpreted as decimal.
When expression a numeric literal if there is a problem with the literal expression javascript would throw a **SyntaxError**.
...
/*
 * exponential base 10 notation denoted as  BeN
...
In Javascript numbers are stored as binary with the [IEEE 754 standard](https://en.wikipedia.org/wiki/IEEE_754) standard with binary64 or double precision format.

Each _Number Data type have 64 bit memory space_ which is equivalent to maximum value of 2^64 number value which is equal to 18 quintillion. That is a huge number
but one bit has to be used to represent the sign of the number , since both positive and negative numbers are represented in **Number** data type, we call that a sign bit. If a sign bit is 0 it means the number is positive and if it's 1 its a negative number.
Also more bit has to be used to represent the decimal point / exponent in numbers which has 11bits in it, we call those exponent bits, its because of those bit that we say in javascript every number is represented as decimal number.
...
For Integer numbers its always safe to assume precise value as long as we don't underpass / overpass the safe integer limit `Number.MAX_SAFE_INTEGER` or `Number.MIN_SAFE_INTEGER` which is about 9 quintillion.
...
Javascript has a wrapper object for numbers data type named **_Number_** which defines some static values and methods.
...
Javascript Number object define two important static number values that are actually not a number called **_NaN_** and **_Infinity_**.
...
A global object always exist in the global scope, it's because of this object that we can access Windows object in browsers and global for Nodejs.
...
To use this function we need to make sure that the value is a number, we can use the `typeof` keyword to check if the value is a number.
...
Infinity arithmetic with a finite number is always Infinity when Infinity is in the left side of the operation , but might change the sign of Infinity.
...
This has been a long article, I hope you enjoyed it. Until next time.
